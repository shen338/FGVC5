...
class InceptionTest(tf.test.TestCase):
  ...
  def testBuildLogits(self):
    ...
    with self.test_session():
      ...
      logits, endpoints = inception_resnet_v2(inputs, num_classes)
  ...
  def testBuildNoClasses(self):
    ...
    with self.test_session():
      inputs = tf.random_uniform((batch_size, height, width, 3))
      net, endpoints = inception_resnet_v2(inputs, num_classes)
      self.assertTrue('AuxLogits' not in endpoints)
  ...
  def testBuildEndPoints(self):
    ...
    with self.test_session():
      inputs = tf.random_uniform((batch_size, height, width, 3))
      _, end_points =inception_resnet_v2(inputs, num_classes)
  ...
  def testBuildAndCheckAllEndPointsUptoPreAuxLogits(self):
    ...
    inputs = tf.random_uniform((batch_size, height, width, 3))
    _, end_points = inception_resnet_v2_base(
  ...
  def testVariablesSetDevice(self):
    ...
    with self.test_session():
      ...
      with tf.variable_scope('on_cpu'), tf.device('/cpu:0'):
        inception_resnet_v2(inputs, num_classes)
  ...
  def testHalfSizeImages(self):
    ...
    with self.test_session():
      inputs = tf.random_uniform((batch_size, height, width, 3))
      logits, end_points = inception_resnet_v2(inputs, num_classes)
      self.assertTrue(logits.op.name.startswith('InceptionResnetV2/Logits'))
      self.assertListEqual(logits.get_shape().as_list(),
                           [batch_size, num_classes])
      pre_pool = end_points['Conv2d_7b_1x1']
  ...
  def testEvaluation(self):
    ...
    with self.test_session() as sess:
      ...
      logits, _ = inception_resnet_v2(eval_inputs,
                                                num_classes,
                                                is_training=False)
  ...
  def testTrainEvalWithReuse(self):
    ...
    with self.test_session() as sess:
      ...
      inception_resnet_v2(train_inputs, num_classes)
